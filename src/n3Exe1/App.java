...
public class App {
	...
	public static void main(String[] args) {
		...
		studentList.add(new Student("Lorem", 20, "Java", 10));
		studentList.add(new Student("Amet", 25, "PHP", 8));
		studentList.add(new Student("Massa", 19, "Java", 9));
		...
		FilterableList<Student, String, Integer> filter3 = (list, ch, len) -> {
			...
			List<Student> result = studentList.stream().filter(s -> 
					s.getStudentNote() >= len &&
					!s.getStudentCourse().equalsIgnoreCase(ch))
					.collect(Collectors.toList());
